...
const Tickets = props => {
    ...
    const getData = () => {
        if (props.location.aboutProps.isManager === 0){
        axios.get(`/api/projects/${props.location.aboutProps.id}/tickets/${props.logIn.userId}`).then(res => {
            setTickets(res.data);
        });
        }
        else {
            axios.get(`/api/projects/${props.location.aboutProps.id}/tickets`).then(res => {
                setTickets(res.data);
            });
        }
    };
